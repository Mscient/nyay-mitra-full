...
import { z } from "zod";
import { eq, and, desc, asc, gte, lte, like, or, count, sum, sql } from "drizzle-orm";
import { db, users, clients, matters, hearings, invoices, tenants, documents } from "@nyay-mitra/database";
...
fastify.get("/v1/workspace/dashboard", { preHandler: authMiddleware }, async (request) => {
  ...
  const pendingInvoicesResult = await db
    ...
    .from(invoices)
    .where(and(
      eq(invoices.advocateId, advocateId),
      or(eq(invoices.status, "DRAFT"), eq(invoices.status, "SENT"), eq(invoices.status, "OVERDUE"))
    ));
...
const clientRoutes = async (fastify: FastifyInstance) => {
  ...
  fastify.get("/", async (request) => {
    const { page, limit, offset } = parsePage(request.query as any);
    const { q } = request.query as any;

    const baseCondition = and(
      eq(clients.advocateId, request.advocateId),
  ...
  fastify.patch("/:id", async (request, reply) => {
    ...
    const { fullName, email, phone, caseTitle, notes } = request.body as any;
...
const matterRoutes = async (fastify: FastifyInstance) => {
  ...
  fastify.get("/:id", async (request, reply) => {
    ...
    if (!matter) return reply.status(404).send({ error: "Matter not found" });
  ...
  fastify.patch("/:id", async (request, reply) => {
    ...
    const [existing] = await db.select({ id: matters.id }).from(matters)
      .where(and(eq(matters.id, id), eq(matters.advocateId, request.advocateId))).limit(1);
    if (!existing) return reply.status(404).send({ error: "Matter not found" });

    const { title, matterType, caseNumber, status, courtCode, feesAgreed } = request.body as any;
    const updates: Record<string, any> = { updatedAt: new Date() };
    if (title) updates.title = title.trim();
    if (matterType) updates.matterType = matterType;
    if (caseNumber) updates.caseNumber = caseNumber;
    if (status) updates.status = status;
    if (courtCode) updates.courtCode = courtCode;
    if (feesAgreed !== undefined) updates.feesAgreed = feesAgreed.toString();
...
const hearingRoutes = async (fastify: FastifyInstance) => {
  ...
  fastify.get("/", async (request) => {
    ...
    if (status) conditions.push(eq(hearings.status, status));
    if (from) conditions.push(gte(hearings.scheduledAt, new Date(from)));
    if (to) conditions.push(lte(hearings.scheduledAt, new Date(to)));
  ...
  fastify.patch("/:id", async (request, reply) => {
    ...
    if (!existing) return reply.status(404).send({ error: "Hearing not found" });

    const { status, nextDate, orderSummary, scheduledAt, judgeName, courtHall } = request.body as any;
    const updates: Record<string, any> = {};
    if (status) updates.status = status;
    if (nextDate) updates.nextDate = nextDate;
    if (orderSummary) updates.orderSummary = orderSummary;
    if (scheduledAt) updates.scheduledAt = new Date(scheduledAt);
    if (judgeName) updates.judgeName = judgeName;
    if (courtHall) updates.courtHall = courtHall;
  ...
  fastify.get("/calendar", async (request) => {
    ...
    const result = await db.select().from(hearings)
      .where(and(
        eq(hearings.advocateId, request.advocateId),
        gte(hearings.scheduledAt, start),
        lte(hearings.scheduledAt, end),
      ))
      .orderBy(asc(hearings.scheduledAt))
      .limit(100);
...
const invoiceRoutes = async (fastify: FastifyInstance) => {
  ...
  fastify.post("/", async (request, reply) => {
    const { clientId, matterId, amount, gstPercent = 18, dueDate, notes } = request.body as any;
...
const documentRoutes = async (fastify: FastifyInstance) => {
  ...
  fastify.post("/", async (request, reply) => {
    const { clientId, matterId, docType, title, htmlContent, aiGenerated } = request.body as any;
    if (!docType || !title) return reply.status(400).send({ error: "docType and title are required" });
...
fastify.get("/v1/workspace/search", { preHandler: authMiddleware }, async (request) => {
  ...
  const [foundClients, foundMatters] = await Promise.all([
    db.select().from(clients).where(and(
      eq(clients.advocateId, request.advocateId),
      or(like(clients.fullName, `%${q}%`), like(clients.caseTitle, `%${q}%`))
    )).limit(5),
    db.select().from(matters).where(and(
      eq(matters.advocateId, request.advocateId),
      like(matters.title, `%${q}%`)
    )).limit(5),
  ]);
