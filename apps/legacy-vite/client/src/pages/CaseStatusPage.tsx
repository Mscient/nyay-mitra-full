...
export default function CaseStatusPage() {
  ...
  async function handleSearch(e?: React.FormEvent) {
    ...
    if (!q || q.length < 10) {
      setError("Please enter a valid CNR number (e.g. MHPN0100012023).");
      return;
    }
    setError("");
    setState("loading");
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(d.error || "Failed to fetch case.");
      }
      const d = await res.json();
      setCaseData(d.data);
      setSource(d.source);
      ...
      setError(err.message || "Could not fetch case details. Please try again.");
      setState("error");
    }
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)", fontFamily: "'Instrument Sans', sans-serif" }}>
      ...
      <nav style={{
        ...
        borderBottom: "1px solid var(--border-gold)", padding: "0 32px",
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "80px 24px 60px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          ...
          <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 42, fontWeight: 500, color: "var(--ink)", lineHeight: 1.1, marginBottom: 12 }}>
            Check Your Case Status
          </h1>
          <p style={{ fontSize: 14, color: "var(--ink-muted)", lineHeight: 1.7, maxWidth: 500, margin: "0 auto" }}>
            Enter your <strong>CNR (Case Number Record)</strong> to instantly get your case status, next hearing date, and full history from the eCourts system.
          </p>
        </div>
        ...
        <form onSubmit={handleSearch} style={{ background: "var(--ivory)", border: "1px solid var(--border-color)", borderRadius: 20, padding: 32, marginBottom: 32 }}>
          <label style={{ display: "block", fontSize: 10, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase", color: "var(--ink-muted)", marginBottom: 8 }}>
            CNR Number
          </label>
          <div style={{ display: "flex", gap: 12 }}>
            <input
              ...
              placeholder="e.g. MHPN0100012023"
              maxLength={20}
              style={{
                flex: 1, padding: "12px 16px", border: "1.5px solid var(--border-color)",
                borderRadius: 10, fontFamily: "monospace", fontSize: 16, letterSpacing: 2,
            ...
            <button
              ...
              style={{
                padding: "12px 28px", background: state === "loading" ? "var(--ink-muted)" : "var(--forest)",
                color: "var(--gold-pale)", border: "none", borderRadius: 10, fontFamily: "'Instrument Sans',sans-serif",
                fontSize: 14, fontWeight: 700, cursor: state === "loading" ? "not-allowed" : "pointer",
                transition: "all 0.2s", letterSpacing: 0.5,
                boxShadow: state === "loading" ? "none" : "0 2px 12px rgba(42,68,42,0.25)"
              }}
            >
          ...
          {error && (
            <div style={{ marginTop: 12, padding: "10px 14px", background: "var(--rust-light)", border: "1px solid rgba(166,58,30,0.2)", borderRadius: 8, fontSize: 13, color: "var(--rust)" }}>
              ⚠️ {error}
            </div>
          )}
        ...
        {state === "loading" && (
          <div style={{ background: "var(--ivory)", border: "1px solid var(--border-color)", borderRadius: 20, padding: 32 }}>
            {[1, 2, 3, 4].map(i => (
              <div key={i} style={{ height: 18, background: "linear-gradient(90deg, var(--cream) 25%, var(--cream-dark) 50%, var(--cream) 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.5s infinite", borderRadius: 6, marginBottom: 16, width: i === 3 ? "60%" : "100%" }} />
            ))}
          </div>
        )}
        ...
        {state === "success" && caseData && (
          <div>
            {isMock && (
              <div style={{ marginBottom: 16, padding: "10px 16px", background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", borderRadius: 10, fontSize: 12, color: "#4338ca", fontWeight: 500 }}>
                📋 Showing sample case data — live eCourts integration requires ECIAPI_TOKEN in your environment.
              </div>
            )}
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                ...
                <div>
                  {(() => {
                    ...
                    return (
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 14px", background: s.bg, borderRadius: 20, fontSize: 12, fontWeight: 700, color: s.text }}>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 20, flexWrap: "wrap" }}>
              <a
                ...
                style={{ padding: "10px 20px", background: "var(--forest)", color: "var(--gold-pale)", border: "none", borderRadius: 10, fontFamily: "'Instrument Sans',sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer", textDecoration: "none" }}
              >
                🔗 View on eCourts Official Portal
              </a>
              <button
                onClick={() => { setCaseData(null); setState("idle"); setCnr(""); }}
                style={{ padding: "10px 20px", background: "var(--cream-dark)", color: "var(--ink-muted)", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer" }}
              >
                ← Search Another Case
              </button>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {["MHPN0100012023", "DLHC0200342024", "GJTK0500122023"].map(ex => (
                <button
                  key={ex}
                  onClick={() => { setCnr(ex); }}
                  style={{ padding: "8px 16px", background: "var(--cream)", border: "1px solid var(--border-gold)", borderRadius: 8, fontFamily: "monospace", fontSize: 13, fontWeight: 600, color: "var(--gold)", cursor: "pointer", letterSpacing: 1 }}
                >
                  {ex}
                </button>
              ))}
