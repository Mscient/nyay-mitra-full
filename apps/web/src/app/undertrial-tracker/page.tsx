...
// AI-GENERATED: Antigravity — Port of UndertrialTrackerPage.tsx from legacy-vite
...
const SECTION_DATA: Array<{ section: string; offence: string; maxYears: number; bailable: boolean }> = [
  ...
  { section: "304", offence: "Culpable Homicide", maxYears: 10, bailable: false },
...
export default function UndertrialTrackerPage() {
  ...
  function calculate() {
    const maxYears = selectedSection ? selectedSection.maxYears : parseFloat(form.customMaxYears) || 0;
    ...
    const daysRemaining = Math.max(0, halfMaxDays === Infinity ? Infinity : halfMaxDays - daysServed);
    ...
    setResult({
      ...
      maxYears, daysServed, halfMaxDays, isEligible, daysRemaining, eligibilityDate,
      isBailable: selectedSection?.bailable || false,
  ...
  async function fetchCaseStatus() {
    ...
    setIsFetchingTracker(true); setTrackerError(null); setTrackerData(null);
    try {
  ...
  const inputStyle: React.CSSProperties = {
    width: "100%", padding: "10px 14px", border: "1px solid var(--border-color)", borderRadius: 8,
    fontFamily: "'Instrument Sans',sans-serif", fontSize: 14, color: "var(--ink)", background: "var(--cream)", outline: "none", boxSizing: "border-box"
  };
  const labelStyle: React.CSSProperties = {
    display: "block", fontSize: 10, fontWeight: 600, letterSpacing: 1.5, textTransform: "uppercase", color: "var(--ink-muted)", marginBottom: 6
  };
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "80px 24px 60px" }}>
        <div style={{ marginBottom: 36, display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 20 }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: "var(--ink-muted)", marginTop: 10, lineHeight: 1.65, maxWidth: 600 }}>Track live district court case statuses or calculate statutory bail eligibility under Section 436A of the CrPC.</p>
          </div>
          <div style={{ display: "flex", background: "rgba(201,146,10,0.1)", borderRadius: 12, padding: 4 }}>
            <button onClick={() => setActiveTab("tracker")} style={{ border: "none", background: activeTab === "tracker" ? "var(--ivory)" : "transparent", color: activeTab === "tracker" ? "var(--forest)" : "var(--ink-muted)", boxShadow: activeTab === "tracker" ? "0 2px 8px rgba(0,0,0,0.05)" : "none", padding: "8px 16px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, transition: "all 0.2s" }}><Search size={14} /> Live Tracking</button>
            <button onClick={() => setActiveTab("calculator")} style={{ border: "none", background: activeTab === "calculator" ? "var(--ivory)" : "transparent", color: activeTab === "calculator" ? "var(--forest)" : "var(--ink-muted)", boxShadow: activeTab === "calculator" ? "0 2px 8px rgba(0,0,0,0.05)" : "none", padding: "8px 16px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, transition: "all 0.2s" }}><Calculator size={14} /> Bail Calculator</button>
          </div>
        </div>
            ...
            <div style={{ background: "var(--ivory)", border: "1px solid var(--border-color)", borderRadius: 20, padding: 32 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div>
                  ...
                  <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Full name" style={inputStyle} onFocus={e => (e.target.style.borderColor = "var(--gold)")} onBlur={e => (e.target.style.borderColor = "var(--border-color)")} />
                </div>
                <div>
                  ...
                  <select value={form.section} onChange={e => setForm(f => ({ ...f, section: e.target.value }))} style={{ ...inputStyle, cursor: "pointer" }}>
                    ...
                    {SECTION_DATA.map(s => <option key={s.section} value={s.section}>Sec {s.section} — {s.offence} ({s.maxYears === 99 ? "Life/Death" : s.maxYears + " yrs"})</option>)}
                ...
                {form.section === "custom" && (
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <div>
                      ...
                      <input type="text" value={form.customSection} onChange={e => setForm(f => ({ ...f, customSection: e.target.value }))} placeholder="e.g., 120B" style={inputStyle} onFocus={e => (e.target.style.borderColor = "var(--gold)")} onBlur={e => (e.target.style.borderColor = "var(--border-color)")} />
                    </div>
                    <div>
                      <label style={labelStyle}>Max Sentence (years)</label>
                      <input type="number" value={form.customMaxYears} onChange={e => setForm(f => ({ ...f, customMaxYears: e.target.value }))} placeholder="e.g., 7" style={inputStyle} onFocus={e => (e.target.style.borderColor = "var(--gold)")} onBlur={e => (e.target.style.borderColor = "var(--border-color)")} />
                    </div>
                  </div>
                )}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div>
                    ...
                    <input type="date" value={form.arrestDate} onChange={e => setForm(f => ({ ...f, arrestDate: e.target.value }))} style={inputStyle} onFocus={e => (e.target.style.borderColor = "var(--gold)")} onBlur={e => (e.target.style.borderColor = "var(--border-color)")} />
                  </div>
                  <div>
                    <label style={labelStyle}>Judicial Remand From</label>
                    <input type="date" value={form.remandDate} onChange={e => setForm(f => ({ ...f, remandDate: e.target.value }))} style={inputStyle} onFocus={e => (e.target.style.borderColor = "var(--gold)")} onBlur={e => (e.target.style.borderColor = "var(--border-color)")} />
                  </div>
                </div>
                <div>
                  <label style={labelStyle}>Court Name</label>
                  <input type="text" value={form.courtName} onChange={e => setForm(f => ({ ...f, courtName: e.target.value }))} placeholder="Sessions Court, Mumbai" style={inputStyle} onFocus={e => (e.target.style.borderColor = "var(--gold)")} onBlur={e => (e.target.style.borderColor = "var(--border-color)")} />
                </div>
                <div>
                  <label style={labelStyle}>Next Hearing Date</label>
                  <input type="date" value={form.nextHearing} onChange={e => setForm(f => ({ ...f, nextHearing: e.target.value }))} style={inputStyle} onFocus={e => (e.target.style.borderColor = "var(--gold)")} onBlur={e => (e.target.style.borderColor = "var(--border-color)")} />
                </div>
              </div>
              <button onClick={calculate} style={{ marginTop: 24, width: "100%", padding: "13px", background: "var(--forest)", color: "var(--gold-pale)", border: "none", borderRadius: 10, fontFamily: "'Instrument Sans',sans-serif", fontSize: 15, fontWeight: 600, cursor: "pointer" }}>⚖️ Check Bail Eligibility</button>
            </div>

            {result && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ borderRadius: 20, overflow: "hidden", border: `2px solid ${result.isEligible ? "#10b981" : "var(--border-gold)"}`, background: "var(--ivory)" }}>
                  <div style={{ padding: "20px 24px", background: result.isEligible ? "#10b981" : result.isLifeSentence ? "var(--ink)" : "var(--forest)", color: "var(--ivory)" }}>
                    ...
                    <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 26, fontWeight: 500 }}>
                      {result.isLifeSentence ? "Life sentence — Sec 436A does not apply" : result.isEligible ? "Eligible for bail under Sec 436A" : "Not yet eligible — still in count-down"}
                    </div>
                  </div>
                  <div style={{ padding: 24 }}>
                    ...
                    {result.eligibilityDate && !result.isEligible && (
                      <div style={{ background: "rgba(196,160,79,0.08)", border: "1px solid var(--border-gold)", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: "var(--ink-mid)" }}>
                    ...
                    {result.nextHearing && (
                      <div style={{ background: "rgba(16,185,129,0.08)", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: "#047857" }}>
                        🗓 Next Hearing: <strong>{new Date(result.nextHearing).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</strong>{result.courtName ? ` — ${result.courtName}` : ""}
                      </div>
                    )}
                    <div style={{ background: "rgba(166,58,30,0.08)", borderRadius: 10, padding: "12px 16px", fontSize: 12, color: "#a63a1e", lineHeight: 1.5 }}>
                ...
                {result.isEligible && (
                  <div style={{ background: "var(--ivory)", border: "1px solid var(--border-color)", borderRadius: 16, padding: 20 }}>
                    ...
                    {["File a bail application under Section 436A CrPC in the Sessions Court", "Attach: Warrant/Remand orders to show custody period", "Cite: Hussainara Khatoon (1979) & SC Re: Prisons (2016) judgments", "Approach DLSA if you don't have a lawyer — they must assist", "NALSA helpline: 15100 | Legal Aid 24×7"].map((item, i) => (
                      <div key={i} style={{ display: "flex", gap: 10, padding: "8px 0", borderBottom: "1px solid var(--border-color)", fontSize: 13, color: "var(--ink-mid)", lineHeight: 1.5 }}>
                        <span style={{ fontWeight: 700, color: "var(--gold)", flexShrink: 0 }}>{i + 1}.</span>{item}
                      </div>
                    ))}
                  </div>
                )}
                <Link href="/chat" style={{ textDecoration: "none" }}>
                  <button style={{ width: "100%", padding: "12px", background: "var(--forest)", color: "var(--gold-pale)", border: "none", borderRadius: 10, fontFamily: "'Instrument Sans',sans-serif", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>💬 Ask AI for More Help</button>
                </Link>
              </div>
            )}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            <div style={{ background: "var(--ivory)", border: "1px solid var(--border-color)", borderRadius: 20, padding: 32 }}>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                <input type="text" value={cnr} onChange={e => setCnr(e.target.value.toUpperCase())} placeholder="e.g., MHAU010002132023" style={{ ...inputStyle, flex: 1, textTransform: "uppercase", letterSpacing: 1 }} onFocus={e => (e.target.style.borderColor = "var(--gold)")} onBlur={e => (e.target.style.borderColor = "var(--border-color)")} />
                <button onClick={fetchCaseStatus} disabled={isFetchingTracker || !cnr.trim()} style={{ padding: "12px 24px", background: "var(--forest)", color: "var(--gold-pale)", border: "none", borderRadius: 8, fontWeight: 600, cursor: isFetchingTracker ? "default" : "pointer", opacity: isFetchingTracker ? 0.7 : 1 }}>{isFetchingTracker ? "Fetching..." : "Track Case"}</button>
              </div>
              {trackerError && <div style={{ color: "#a63a1e", fontSize: 13, marginTop: 12, display: "flex", alignItems: "center", gap: 6 }}><AlertCircle size={14} /> {trackerError}</div>}
            </div>

            {trackerData && (
              <div style={{ background: "var(--ivory)", border: "1px solid var(--border-color)", borderRadius: 20, overflow: "hidden" }}>
                <div style={{ background: "var(--forest)", padding: "24px 32px", color: "var(--ivory)", display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
                  <div>
                  ...
                  <div style={{ background: "rgba(255,255,255,0.1)", padding: "8px 16px", borderRadius: 20, fontSize: 12, fontWeight: 600 }}>Status: {trackerData.status}</div>
                </div>
                <div style={{ padding: 32, display: "grid", gridTemplateColumns: "1fr 2fr", gap: 40, '@media (max-width: 768px)': { gridTemplateColumns: "1fr" } } as React.CSSProperties}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                    ...
                    <div style={{ background: "var(--cream)", borderRadius: 12, padding: 16 }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 600, color: "var(--ink)" }}>{trackerData.next_hearing_date ? new Date(trackerData.next_hearing_date).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" }) : "Not Scheduled"}</div>
                    </div>
                    <div style={{ background: "var(--cream)", borderRadius: 12, padding: 16 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 0, position: "relative" }}>
                      {trackerData.history?.map((event: any, idx: number) => {
                        ...
                        return (
                          <div key={idx} style={{ display: "flex", gap: 20, position: "relative" }}>
                            {!isLast && <div style={{ position: "absolute", left: 11, top: 24, bottom: -8, width: 2, background: "rgba(196,160,79,0.2)", zIndex: 0 }} />}
                            ...
                            <div style={{ flex: 1, paddingBottom: 32 }}>
                              <div style={{ fontSize: 12, fontWeight: 600, color: "var(--gold)", marginBottom: 2 }}>{new Date(event.date).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</div>
                      ...
                      {(!trackerData.history || trackerData.history.length === 0) && (
                        <div style={{ fontSize: 14, color: "var(--ink-muted)" }}>No case history available.</div>
                      )}
                    </div>
                  </div>
                </div>
